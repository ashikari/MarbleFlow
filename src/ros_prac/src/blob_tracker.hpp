...
void preprocess(cv::Mat& frame, cv::Mat& grayFrame){
        ...
        cv::dilate(grayFrame, grayFrame, element,cv::Point(-1,-1), 15);
        cv::GaussianBlur(grayFrame, grayFrame, cv::Size(7,7), 3, 3);
        cv::erode(grayFrame, grayFrame, element, cv::Point(-1,-1), 15);
