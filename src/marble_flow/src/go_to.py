...
# Adapted from go_to_*.py functions in intera_sdk/intera_examples
# Contains functions for joint angles, "angles in contact", and cartesian pose. 
# Also contains classes to be used as arguments for these functions. 
...
class joint_angle_arg():
    def __init__(self, joint_angles = [0.0, -0.9, 0.0, 1.8, 0.0, -0.9, 0.0], speed_ratio = 0.5, accel_ratio = 0.5, timeout = None):
...
def joint_angles_in_contact(input_arg):
    ...
    arg_fmt = argparse.RawDescriptionHelpFormatter
    parser = argparse.ArgumentParser(formatter_class=arg_fmt,
                                     description="joint_angles_in_contact.__doc__")
    parser.add_argument(
        "-q", "--joint_angles", type=float,
        nargs='+', default=[0.0, -0.9, 0.0, 1.8, 0.0, -0.9, 0.0],
        help="A list of joint angles, one for each of the 7 joints, J0...J6")
...
class cartesian_pose_arg():
    def __init__(self, position = None, orientation = None, relative_pose = None,
